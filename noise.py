...
class AutonomousMind:
    ...
    def cognitive_cycle(self):
        ...
        while self.running:
            ...
            # 生成基础信号
            self.signal_phase += 0.1
            base_signal = (
                np.sin(np.linspace(0, self.signal_phase, 500)) 
                + 0.5 * np.sin(3 * np.linspace(0, self.signal_phase, 500))
            )
            
            # 添加好奇心驱动的随机探索
            if any(net.curiosity > 0.7 for net in self.conscious.networks):
                exploration_factor = max(net.curiosity for net in self.conscious.networks)
                observation = base_signal + np.random.normal(0, 0.3 * exploration_factor, 500)
            else:
                observation = base_signal + np.random.normal(0, 0.2, 500)
    ...
    def update_goals(self, net):
        ...
        if net.goal is None and random.random() < 0.02:
            goal_types = ["最小化损失", "最大化情感", "增强连接", "提高警觉"]
            net.goal = {
